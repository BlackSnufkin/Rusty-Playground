...
use windows::{
    core::{Error, GUID, HSTRING, PCWSTR, PWSTR},
    Win32::{
...
fn open_wlan_handle(api_version: u32) -> Result<HANDLE, Error> {
    let mut negotiated_version = 0;
    let mut wlan_handle = HANDLE(0);
...
fn main() {
    ...
    println!("\nPress enter to exit...");
    io::stdout().flush().unwrap(); // Make sure 'Press enter to exit...' is displayed before waiting for input
    io::stdin().read_line(&mut String::new()).unwrap();

    // Your existing unsafe clean-up code
